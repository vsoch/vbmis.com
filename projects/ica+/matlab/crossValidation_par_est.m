...
function [wopt,best_cva,opt_lambda,opt_alpha,fit_opt] = crossValidation_par_est(X,y,K,Lambda,alphav)
...
for fold = 1:K
   ...
   for a = 1:length(alphav)   %grid search for alpha
       ...
       Indxc = Ytest == Yest;
       cva = sum(Indxc,1)./length(ytest);                 %cross
% validation accuracy
...
fit_opt = glmnet(X,y,'binomial',options);
